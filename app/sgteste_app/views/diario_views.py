...
def lista_execucao(request, projeto_id):
    if request.method == 'POST':
        if get_ct_restante(projeto_id) == 0:
            ...
            tag_inicial = '<h2>'
            tag_final = '</h2>'
            msg1 = 'Permissão Negada. Não é possivel concluir projeto com ct pendente'
            tag_br = '<br>'
            tag_a = '<a href="/diario-execucao/' + str(projeto_id) + '"' + '>Voltar</a>'
            return HttpResponseForbidden(tag_inicial + msg1 + tag_final + tag_br + tag_a)
    else:
        ...
        return render(request, 'diario/lista-execucao.html', {'diario': diario, 'projeto_id': projeto_id, 'projeto': projeto, 'diferenca_cts': diferenca_cts, 'dias_adicionais': dias_adicionais, 'prj': prj, 'cts_pendentes': cts_pendentes})
...
def executar_teste(request, pk, projeto_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if executados != '0':
                status_prj = Projeto.objects.get(pk=projeto_id).status_projeto_id
                if status_prj != 2:
        ...
        return render(request, 'diario/executar-teste.html', {'form': form, 'diario': diario})


def adicionar_planejamento(request, projeto_id):
    projeto = Projeto.objects.get(pk=projeto_id).id
    data = Diario.objects.filter(projeto_id=projeto).order_by('-data_execucao')[0].data_execucao
    ...
    return render(request, 'diario/adicionar-execucao.html', {'form': form, 'projeto': projeto})


def concluir_projeto(Object, object_id):
    data_exec = Diario.objects.filter(projeto_id=object_id).order_by('-data_execucao')[0].data_execucao
    Object.objects.filter(pk=object_id).update(data_conclusao=dt.strftime(data_exec, '%Y-%m-%d'))
